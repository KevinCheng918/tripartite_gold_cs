...
題庫從 85 題長到 130 幾題之後，要找某一題還得先想它在哪個類別 ——
所以右欄加了搜尋框，**跨類別**過濾。

純前端實作：`ajax-list` 本來就一次把所有類別與題目載進 `categories`，
再打一次後端只會更慢。

### 編號與關鍵字共用同一個輸入框

```
23     → 先列出 #23，後面接內容含「23」的題目
#23    → 同上，開頭的井字號會被去掉（方便直接貼清單）
加簽   → 標題或答案含「加簽」的題目
10003  → 內容含 10003 的題目
```

**輸入純數字時不能只查編號** —— 錯誤碼本身就是數字（`10003`、`-4`），
只比對編號的話，想找錯誤碼反而搜不到。所以數字會同時比對編號與內容，
編號完全相符的排最前面（`searchItems()` 的 `byId` / `byText` 兩個陣列）。
...
| 範圍 | 選到的那一類 | 跨全部類別 |
| 標示 | `#編號` + 標題 | `#編號` + **所屬類別** + 標題 |
| 新增鈕 | 顯示 | 隱藏（沒有「目前類別」可歸屬） |
| 拖曳排序 | 可用 | **停用**，排序只在類別內有意義 |
| 編輯／刪除 | 可用 | 可用（用 `findItem()` 跨類別找，不能只查目前類別） |

題目列表也補上了 `#編號` 顯示 —— 以前只有 `data-id`，畫面上看不到，
拿著編號清單對不起來。
