...
> ⚠️ **這份文件有兩段已經被改掉了**，以 [auto-reply-natural.md](auto-reply-natural.md) 為準：
>
> 1. **反問編號選項已移除**。上線後發現「請問是以下哪一項？1. 2. 3.」
>    等於強迫客人從題庫裡挑一項，客人明明講的是別的事。
>    現在不確定就說稍等並轉人工。
> 2. **模型現在可以生成「承接句」**（回應客人的那一兩句）。
>    答案本體仍然是題庫原文、一字不改，但開場白不再是固定模板。
>
> 另外新增了 **intent 判斷**：客人不是只會問問題，也會提需求、會寒暄。
> 需求與寒暄**完全不查題庫**。
>
> 下面凡是提到 `clarify` / `candidate_ids` / 固定開場白的段落都已失效。
...
題庫有十幾題是三、四百字的完整說明（錯誤碼判斷、必填欄位、L1～L4 流程）。
一整塊丟過去，客人會跳著看，最後那段「麻煩提供代理帳號與訂單號」常常整個被略過 ——
**而那才是我們真正需要他做的事。**

`AutoReplyService::splitContent()` 依段落把答案切成多則。
實際分佈：132 題裡 117 題維持一則，10 題切兩則，5 題切三則。

| 設定（`auto_reply.answer`） | 值 | 作用 |
|---|---|---|
| `chunk_chars` | 300 | 累積超過就斷在下一個段落邊界 |
| `min_tail` | 80 | 最後一則短於此併回前一則，免得結尾那句問候自己佔一則 |

三條規則，順序就是它們的重要性：

1. **只在空行切。** 寧可某一則長一點，也不要把一句話砍成兩半 ——
   切在句子中間比不切還糟。所以單一段落本身就超過上限時，那一則就是會比較長。
2. **標題行不留在結尾。** 斷點剛好落在「判斷卡在哪一段：」後面時，
   標題會孤零零留在上一則、清單跑到下一則，比不分則還難讀。
   `carryHeading()` 把它帶到下一則開頭。
   認得兩種寫法：冒號結尾、`【】` 開頭，且長度要在 30 字內 ——
   一整段說明文字剛好以冒號結尾的情況是有的，那種不該被搬走。
3. **署名與已回覆標記只掛最後一則。** 每則都接一個 `-A`，客人會看到三個署名。

> ⚠️ **中間那則送失敗就整個停下**，不要繼續送後面的。
> 客人收到跳掉一段的說明，比整個沒收到更難處理。

### 沒有一起做的

- **快速回覆直送**（聊天視窗點題庫 → 直接送出）仍是一整塊。
  那條走的是一般的 `sendReply`，跟客服自己打的字無法區分，
  而客服手打的內容絕對不該被自動分則。要做的話得在 API 上標記「這是題庫答案」。
- **後台預覽**（`preview()`）顯示的仍是完整一段，沒有反映分則。
  預覽是拿來確認答案內容的，分不分則不影響判斷。

### 另一個方向（沒採用，但仍可疊上來）

把那 14 題改寫成倒金字塔 —— 客人要做的事放最前面、背景知識沉到最後，
重點用 `<b>`、欄位名與錯誤碼用 `<code>`。
（`TelegramBotService::escapeHtml()` 的白名單本來就有 `b / i / u / s / code / pre / a`，
HTML 標籤可以直接寫在題庫答案裡。不能用的是 Markdown 星號，那個不會被渲染。）

分則是排版層的處理，改寫是內容層的，兩者不衝突。
