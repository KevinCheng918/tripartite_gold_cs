...
class QuickReplyKnowledgeSeeder extends Seeder
{
    ...
    private const RETIRED_KEYS = [
        ...
        'order.test_reversal_deposit'   => '沖正是一整套流程，已合併回 order.test_reversal',
        'order.test_reversal_withdraw'  => '同上',
        'order.test_reversal_credit'    => '同上',

        /*
         * 以下是題庫本來就有、而且寫得比這份 seeder 準確的主題。
         *
         * 既有題目是客服實戰累積的，後台路徑正確；這份 seeder 的路徑是
         * 從程式碼推的，有猜錯（例如 IP 白名單其實在「商戶代理總覽 →
         * 操作選擇 → 修改」，不是「會員管理」）。同一個主題留兩題只會
         * 讓自動回覆比對時猶豫，反而兩邊都命不中。
         */
        'api.environment'    => '題庫已有 #21／#36／#37，且複製流程更正確',
        'error.unauthorized' => '題庫已有 #40',
        'error.throttle'     => '題庫已有 #44',
        'error.ip_denied'    => '題庫已有 #22，後台路徑更正確',
        'error.blacklist'    => '題庫已有 #43／#48／#49',
    ];
...
TEXT,
            ...
            [
                /*
                 * 沖正是一整套連貫的 SOP，刻意**不拆成多題** ——
                 * 拆開的話客人問一次只拿到三分之一，還要再問兩次。
                 * 這是少數「一題講完整流程」比「一題一件事」更合適的情況。
                 */
                'key'      => 'order.test_reversal',
                'category' => '訂單',
                'label'    => '餘額沖正要怎麼做？商戶測試完畢後要處理什麼？',
                'answer'   => <<<'TEXT'
商戶串接測試完畢後，需要把測試產生的訂單與餘額沖正才能正式開跑。
沒有沖正的話，這些測試單之後仍會進入餘額下放，造成對帳金額對不起來。

【一、代收單處理】

• 狀態為待處理：點選該筆的「狀態設定」，改為「作廢」
• 狀態為成功：點選該筆的「退款」，系統會產生一筆負數的退款紀錄，
  例如 -1000.000000(refund)，代表該筆金額已經沖回

【二、代付單處理】

• 狀態為待處理：點選該筆的「狀態設定」，改為「失敗」
• 狀態為成功：預設無法直接改成失敗，需要依序完成三個步驟
...
扣除成功後，系統 BOT 會推送一則後台操作訊息，內容包含操作者、
商店號、當前點數、扣除點數、最後點數與備註，可以用來核對。
調整紀錄也會留在「餘額歷程」中。
...
1. 通道是否已開啟
   路徑：商戶代理總覽 → 設定選擇 → 支付管道設定
2. 通道的水位與筆數是否已滿
3. 商戶在該通道的金額上下限
4. 商戶本身的金額上下限

若該商戶名下沒有任何可用通道，請先到「系統管理 → 支付帳戶設定」新增，
再回到商戶代理總覽開啟。

以上都確認過仍無法排除，歡迎提供截圖，我們協助您一起確認。
TEXT,
            ],
            [
