(function () {
    ...
    statusMap[2] = { text: i18n.status_lock, css: 'badge--pending' };
    statusMap[0] = { text: i18n.status_deactivate, css: 'badge--rejected' };

    function renderTable(accounts) {
        // 不列出管理者帳號（level=0）
        accounts = accounts.filter(function (a) { return a.level !== 0; });

        var rows = accounts
            .map(function (account) {
                var levelLabel = account.level === 0
                    ? '<span class="badge badge--approved">' + i18n.level_admin + '</span>'
                    : '<span class="badge badge--pending">' + i18n.level_cs + '</span>';
    ...
    function openEditModal(id) {
        var account = accountsData.filter(function (a) { return a.id === id; })[0];
        if (!account) { return; }
    ...
    function submitEditAccount(e) {
        ...
        var data = { nickname: document.getElementById('edit-nickname').value };
    ...
    function openAssignPermissionsModal(userId) {
        ...
        var account = accountsData.filter(function (a) { return a.id === userId; })[0];
