<?php

return [
    'nav_label' => 'Accounts',
    'page_title' => 'Accounts',
    'subtitle'   => 'Manage accounts and permissions',
    'deleted' => 'Account deleted',
    'created' => 'Account created',
    'updated' => 'Account updated',
    'field_account' => 'Account',
    'field_nickname' => 'Nickname',
    'field_telegram_nickname' => 'Telegram signature',
    'label_telegram_nickname' => 'TG sign',
    'telegram_nickname_hint'  => 'Messages this account sends in Telegram chats will end with "-nickname". Leave blank to disable.',
    'telegram_nickname_ph'    => 'e.g. Amy',
    'field_telegram_username' => 'Telegram username',
    'telegram_username_hint'  => 'Used to @ this person in the internal group when an auto-reply ticket times out. Leave blank to never be tagged.',
    'telegram_username_ph'    => 'e.g. amy_chen (without @)',
    'not_set'                 => 'Not set',
    'no_data'                 => 'No data',
    'no_log'                  => 'No records',
    'load_failed'             => 'Failed to load',
    'filter_all'              => 'All',
    'action_search'           => 'Search',
    'action_reset'            => 'Reset',
    'action_collapse'         => '— Collapse —',
    'action_expand'           => '— Expand —',
    'field_action'            => 'Actions',
    'field_project'           => 'Projects',
    'stat_total'              => 'Total',
    'unit_person'             => 'people',
    'log_os'                  => 'OS',
    'log_browser'             => 'Browser',
    'log_device'              => 'Device',
    'field_password' => 'Password',
    'field_level' => 'Role',
    'level_admin' => 'Admin',
    'level_boss' => 'Boss',
    'level_leader' => 'Leader',
    'level_engineer' => 'Engineer',
    'level_cs' => 'CS',
    'field_status' => 'Status',
    'status_normal' => 'Normal',
    'status_lock' => 'Locked',
    'status_deactivate' => 'Deactivated',
    'status_normal_hint' => 'Full access, can sign up and swap shifts',
    'status_lock_hint' => 'Can login, but cannot sign up or swap shifts — shifts assigned by admin',
    'status_deactivate_hint' => 'Account disabled, cannot login',
    'action_create' => 'New account',
    'action_edit' => 'Edit',
    'action_save' => 'Save',
    'action_delete' => 'Delete',
    'action_change_status' => 'Change status',
    'action_assign_permissions' => 'Permissions',
    'password_hint' => 'Leave blank to keep unchanged',
    'field_level' => 'Level',
    'level_admin' => 'Admin',
    'level_cs' => 'CS',

    'msg' => [
        'required'             => 'This field is required',
        'unique'               => 'This account already exists',
        'regex_account'        => 'Account must be 4-20 half-width alphanumeric or underscore characters',
        'regex_password'       => 'Password must be at least 8 half-width characters (letters, numbers, symbols)',
        'full_width_password'  => 'Password contains full-width or unsupported characters (including spaces). Use half-width letters, numbers and symbols.',
        'full_width_account'   => 'Account contains full-width or unsupported characters. Use half-width letters, numbers and underscore.',
        'max_string'           => 'Must not exceed :value characters',
        'telegram_nickname_max' => 'Telegram signature must not exceed :value characters',
        'telegram_username_invalid' => 'Invalid Telegram username (5-32 characters, letters, digits and underscore only)',
        'invalid_status'       => 'Invalid status value',
        'permissions_required' => 'Please select permissions',
        'invalid_permission'   => 'Invalid permission value',
        'create_failed'        => 'Failed to create account',
        'update_failed'        => 'Failed to update account',
        'assign_failed'        => 'Failed to assign permissions',
    ],
];
