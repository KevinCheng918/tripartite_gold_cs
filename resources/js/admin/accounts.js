(function () {
    ...
    function renderTable(accounts) {
        const rows = accounts
            .map(function (account) {
                const roles = account.roles.map(function (role) {
                ...
                return (
                    ...
                    '<td>' + account.name + '</td>' +
                    '<td>' + account.email + '</td>' +
                    '<td>' + (account.status === 1 ? i18n.status_active : i18n.status_disabled) + '</td>' +
                    '<td>' + roles + '</td>' +
                    '<td>' +
                    '<button class="js-assign-roles">' + i18n.action_assign_roles + '</button> ' +
        ...
        root.innerHTML =
            '<button id="js-create-account">' + i18n.action_create + '</button>' +
            '<table><thead><tr>' +
            '<th>' + i18n.field_name + '</th><th>' + i18n.field_email + '</th>' +
            '<th>' + i18n.field_status + '</th><th>' + i18n.field_roles + '</th><th></th>' +
    ...
    function loadAccounts() {
        apiFetch('/admin/accounts/ajax-list')
            .then(function (body) {
    ...
    function createAccount() {
        const name = prompt(i18n.field_name);
        if (!name) {
            return;
        }
        const email = prompt(i18n.field_email);
        const password = prompt(i18n.field_password);
    ...
    function deleteAccount(id) {
        ...
        apiFetch('/admin/accounts/ajax-delete/' + id, { method: 'DELETE' })
            .then(loadAccounts)
            .catch(function (error) {
    ...
    function assignRoles(id) {
        const input = prompt('Role IDs, comma separated');
        if (input === null) {
            return;
        }
        const roleIds = input
